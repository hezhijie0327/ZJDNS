...
// uses semantic equivalence (ignoring TTL, RR order, and EDNS padding).
package defense
...
// Uses miekg/dns' tab-separated String() format: name\tTTL\tclass\ttype\trdata.
func rdataString(rr dns.RR) string {
	s := rr.String()
	// Skip 4 tab-separated fields (name, TTL, class, type) to reach rdata.
...
func CollectAndVote(responses []*dns.Msg, threshold int) *dns.Msg {
	...
	if threshold <= 1 {
		return responses[0] // threshold=1: first response wins
	}
