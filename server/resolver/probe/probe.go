...
type CacheSetter interface {
	Set(qname string, qtype, qclass uint16, ecs *edns.ECSOption, dnssecOK bool, answer, authority, additional []dns.RR, validated bool, rcode uint16) int64
...
func ProbeNSAddrs(ctx context.Context, cache CacheSetter, addrs []string) {
	...
	prober := latency.New(defaultNSProbeSteps(), ctx)
	defer prober.Close()
