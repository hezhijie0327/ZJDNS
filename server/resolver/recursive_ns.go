...
func (r *Recursive) cacheGlueRecords(glue map[string][]dns.RR) {
	...
	for nsName, records := range glue {
		...
		if len(aGlue) > 0 {
			r.cache.Set(nsName, dns.TypeA, dns.ClassINET, nil, false, aGlue, nil, nil, false, dns.RcodeSuccess)
		}
		if len(aaaaGlue) > 0 {
			r.cache.Set(nsName, dns.TypeAAAA, dns.ClassINET, nil, false, aaaaGlue, nil, nil, false, dns.RcodeSuccess)
		}
	}
