	...
	for _, ns := range nsRecords {
		...
		g.Go(func() error {
			...
			if r.cache != nil && len(ansARecords) > 0 {
				r.cache.Set(nsName, dns.TypeA, dns.ClassINET, nil, false, ansARecords, nil, nil, false, dns.RcodeSuccess)
			}
			if r.cache != nil && len(ansAAAARecords) > 0 {
				r.cache.Set(nsName, dns.TypeAAAA, dns.ClassINET, nil, false, ansAAAARecords, nil, nil, false, dns.RcodeSuccess)
			}
