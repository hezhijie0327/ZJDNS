...
// UDP socket.  dtlcp.Dial is not used because it creates a connected socket
// via net.Dial, which is incompatible with the library's internal WriteTo calls.
//
...
// DNS messages are framed with a 2-byte big-endian length prefix, same as
...
func (c *Client) executeDTLCP(ctx context.Context, msg *dns.Msg, server *config.UpstreamServer) (*dns.Msg, error) {
	...
	// Read full DTLCP record containing [2-byte prefix][DNS response].
