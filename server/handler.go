...
// queryMetrics bundles the per-query state needed for deferred metric recording.
...
// ProcessQuery is the exported entry point for the DNS query pipeline.
// Used by benchmarks and tests that need direct access to the pipeline
// without going through dnsproxy's Handler interface.
func (s *Server) ProcessQuery(req *dns.Msg, clientIP net.IP, isSecureConnection bool, requestProtocol string) *dns.Msg {
	return s.processDNSQuery(req, clientIP, isSecureConnection, requestProtocol)
}
