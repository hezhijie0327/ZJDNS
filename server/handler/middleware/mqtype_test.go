...
func TestMQTYPE_Merge_FiltersDNSSECForDO0(t *testing.T) {
	...
	// Set serves both DO subtests below.
...
// TestMQTYPE_ForwardingMode_MergesLocally verifies forwarding mode merges
// locally too — ZJDNS is a full resolver, so the response supports MQTYPE
// even when no hop in the upstream chain does.  The option is never passed
// through.
func TestMQTYPE_ForwardingMode_MergesLocally(t *testing.T) {
	store := mqTestStore(t)
	defer func() { _ = store.Close() }()

	fake := &fakeMQResolver{
		servers: 1, // upstream configured → forwarding mode
		results: map[uint16]*resolver.QueryResult{
			dns.TypeAAAA: {Answer: []dns.RR{aaaaRecord("2001:db8::1")}, Cacheable: true},
		},
	}
	...
	if len(qctx.Res.Answer) != 2 {
		t.Errorf("answer count = %d, want 2 (A + AAAA — forwarding must merge locally)", len(qctx.Res.Answer))
	}
	var mqFound bool
	for _, rr := range qctx.Res.Pseudo {
		if mqr, ok := rr.(*dns.MQRESPONSE); ok {
	...
	if !mqFound {
		t.Error("forwarding mode must return MQTYPE-Response (local merge)")
	}
}
