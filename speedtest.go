...
func (st *SpeedTester) initICMP() {
	...
	if err == nil {
		...
		if strings.Contains(err.Error(), "operation not permitted") {
			...
			conn4UDP, err := icmp.ListenPacket("udp4", "")
			if err == nil {
				st.icmpConn4 = conn4UDP
				writeLog(LogDebug, "📍 速度测试: 使用udp4作为IPv4 ICMP备选方案")
			}
		}
	}
	...
	if err == nil {
		...
		if strings.Contains(err.Error(), "operation not permitted") {
			...
			conn6UDP, err := icmp.ListenPacket("udp6", "")
			if err == nil {
				st.icmpConn6 = conn6UDP
				writeLog(LogDebug, "📍 速度测试: 使用udp6作为IPv6 ICMP备选方案")
			}
		}
	}
}
...
// CheckAndSortRoutes 对DNS响应中的A/AAAA记录进行测速并排序
func (st *SpeedTester) CheckAndSortRoutes(response *dns.Msg) *dns.Msg {
	if response == nil {
	...
	writeLog(LogDebug, "📍 速度测试: A记录数=%d, AAAA记录数=%d", len(aRecords), len(aaaaRecords))
...
func (st *SpeedTester) pingWithICMP(ip string, timeout time.Duration) time.Duration {
	...
	if err != nil {
		// 如果直接写入失败，尝试使用网络连接方式
		writeLog(LogDebug, "📍 速度测试: 直接写入ICMP消息失败 %s: %v，尝试网络连接方式", ip, err)

		// 创建网络连接
		netConn, dialErr := net.DialTimeout(networkType, ip, timeout)
		if dialErr != nil {
			writeLog(LogDebug, "📍 速度测试: 无法建立网络连接到 %s: %v", ip, dialErr)
			return -1
		}
		// 忽略关闭连接的错误
		...
		// 通过网络连接发送
		_, writeErr := netConn.Write(wb)
		if writeErr != nil {
			writeLog(LogDebug, "📍 速度测试: 通过网络连接发送ICMP消息失败 %s: %v", ip, writeErr)
			return -1
		}
	}
