...
**Client names** (NextDNS-style identity tokens, always on): HTTP-based listeners accept `{endpoint}/{name}` paths (unknown extra segments 404) and fall back to SNI; TLS/QUIC listeners extract `{name}.{domain}` from the handshake SNI (plain UDP/TCP and DNSCrypt have no name channel; DTLS is excluded — pion does not expose SNI post-handshake). Names are strict `[a-z0-9][a-z0-9-]{0,62}` (case-folded; dots/slashes/underscores rejected, so a mistyped CIDR fails config load instead of becoming a name), extracted in `internal/dnsutil/clientname.go`, and flow through `edns.RequestMeta.ClientName` into `qctx.ClientName`. Self-signed certificates carry a `*.{domain}` wildcard SAN so the SNI form validates. An ACL entry that is not an IP/CIDR is a client name; semantics are allow-wins — a presented name in `allow` grants access regardless of a denied IP range (the name is the explicit credential, the range block holds for anonymous traffic). `config.OverlapEntries` warns at startup when an allow entry silently overrides a deny entry.
