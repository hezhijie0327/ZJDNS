...
这套方法曾发现并修复过五个真实问题（见文末「已发现问题」），可直接复用。
...
| **bogus 缓存 + sticky EDE** | `dnssec_enforce` 实例命中未验证缓存绕过 enforce；正常签名域显示 EDE 6（DNSSEC Bogus）且无缓存 | (a) bogus 结果写入共享缓存后 enforce 实例直接命中 (b) 中间级验证失败的 `chain.lastEDECode` 粘滞到最终 validated 响应 | (a) `dnssecCacheable` 禁止缓存 unvalidated+bogus-EDE (b) 验证成功时清零 `lastEDECode` → 基线 0 失败（2026-08-08 基线：tls=15 fail 属测量噪声，其余协议 0） |
