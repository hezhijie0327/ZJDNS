...
function DownloadConfiguration() {
    ...
    fi && curl ${CURL_OPTION:--4 -s --connect-timeout 15} "https://source.zhijie.online/CMA_DNS/main/config.json" > "${DOCKER_PATH}/conf/config.json" && sed -i "s/demo.zhijie.online/${CUSTOM_DOMAIN:-demo.zhijie.online}/g;s/\"\\/\"/${CUSTOM_PATH:-\\/}/g;s/99235a6e-05d4-2afe-2990-5bc5cf1f5c52/${CUSTOM_UUID:-$(uuidgen | tr 'A-Z' 'a-z')}/g" "${DOCKER_PATH}/conf/config.json"
}
