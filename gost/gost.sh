...
function CreateNewContainer() {
    if [ "${RUNNING_MODE:-server}" == "server" ]; then
        ...
        if [ "${GOST_DYNAMIC_IP}" != "" ]; then
            GOST_IP=(
                $(dig @$(echo ${CDN_ENDPOINT} | cut -d ',' -f 2) +short A $(echo ${CDN_ENDPOINT} | cut -d ',' -f 1) | grep -E "^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$" | sort -n | uniq | awk "{ print $2 }")
                $(dig @$(echo ${CDN_ENDPOINT} | cut -d ',' -f 2) +short AAAA $(echo ${CDN_ENDPOINT} | cut -d ',' -f 1) | grep -E "^(([0-9a-f]{1,4}:){7,7}[0-9a-f]{1,4}|([0-9a-f]{1,4}:){1,7}:|([0-9a-f]{1,4}:){1,6}:[0-9a-f]{1,4}|([0-9a-f]{1,4}:){1,5}(:[0-9a-f]{1,4}){1,2}|([0-9a-f]{1,4}:){1,4}(:[0-9a-f]{1,4}){1,3}|([0-9a-f]{1,4}:){1,3}(:[0-9a-f]{1,4}){1,4}|([0-9a-f]{1,4}:){1,2}(:[0-9a-f]{1,4}){1,5}|[0-9a-f]{1,4}:((:[0-9a-f]{1,4}){1,6})|:((:[0-9a-f]{1,4}){1,7}|:)|fe80:(:[0-9a-f]{0,4}){0,4}%[0-9a-z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-f]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))$" | sort -n | uniq | awk "{ print $2 }")
                ${GOST_IP[@]}
            )
        fi
