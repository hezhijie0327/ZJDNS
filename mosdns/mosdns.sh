...
function UpdateGeoSiteRule() {
    ...
    curl -s --connect-timeout 15 "${GHPROXY_URL}https://raw.githubusercontent.com/hezhijie0327/GFWList2AGH/main/gfwlist2domain/blacklist_full.txt" > "${DOCKER_PATH}/data/GeoSite_PROXY_hezhijie0327.txt"
    curl -s --connect-timeout 15 "${GHPROXY_URL}https://raw.githubusercontent.com/hezhijie0327/GFWList2AGH/main/gfwlist2domain/whitelist_full.txt" > "${DOCKER_PATH}/data/GeoSite_DIRECT_hezhijie0327.txt"
...
function UpdateSuspiciousIPRule() {
    curl -s --connect-timeout 15 "${GHPROXY_URL}https://raw.githubusercontent.com/hezhijie0327/CMA_DNS/main/mosdns/POLLUTED.txt" > "${DOCKER_PATH}/data/SuspiciousIP_POLLUTED.txt"
    curl -s --connect-timeout 15 "${GHPROXY_URL}https://raw.githubusercontent.com/hezhijie0327/CMA_DNS/main/mosdns/RESERVED.txt" > "${DOCKER_PATH}/data/SuspiciousIP_RESERVED.txt"
}
