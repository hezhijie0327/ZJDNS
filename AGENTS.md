...
| **No-cache marking** | UDP upstream (hopguard/capsguard) | Responses served without a positive defense verification (hopguard baseline learning / TTL capture unavailable; capsguard unrandomized §6.4 retry, downgraded address, PTR exemption) carry the ZJDNS-private EDE 65281 (`edns.EDEZJDNSDefenseUncertain`) and are never cached — a poisoned answer's blast radius is one query, not its TTL. An armed hopguard fingerprint or a successful 0x20 echo clears the mark. Downstream ZJDNS instances refuse to cache marked responses (same receive-side refusal as 65280). TCP/stream transports are deterministic and never marked |
...
| [docs/rfc/](docs/rfc/) | Mirrored RFCs and drafts (125 txt files) |
