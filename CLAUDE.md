...
- **Summary stats**: `Store.Summary()` queries entries + hit_counters via JOINs. Returns entries, hits, avg response time, per-protocol hits, rcode distribution, hijack/fallback/prefetch/stale/rewrite counts. Queryable anytime via `dig zjdns.stats CH TXT`. Additional dynamic queries: `zjdns.uptime` (server uptime), `zjdns.goroutines` (goroutine count), `zjdns.memstats` (heap alloc/objects/GC). All wired via rewrite DynamicContent in `server.New()` before `LoadRules()`.
