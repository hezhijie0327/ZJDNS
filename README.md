...
[![Version](https://img.shields.io/badge/Version-3.8.14-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
### DNS 解析
- **递归解析**：从 IANA 根服务器逐步解析至权威，完整 DNSSEC 信任链（根提示 + 延迟排序 NS 缓存）
- **上游转发**：主/备并发查询 + 首胜策略；支持 `protocol: "recursive"` 纯递归模式
...
- **DNS Cookie**：SipHash-2-4（[RFC 9018](docs/rfc/rfc9018.txt)），24h 密钥轮换，保留历史密钥兼容慢客户端
- **EDNS Padding**：[RFC 8467](docs/rfc/rfc8467.txt)，请求 128/响应 468 块对齐，随机填充
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `address` | string | `host:port` / `sdns://` |
| `protocol` | string | `udp`/`tls`/`quic`/`https`/`http3`/`dtls`/`dnscrypt`/`tlcp`/`http-tlcp`/`dtlcp`/`recursive`，`sdns://` 自动推导 |
...
| `poisonguard`/`spoofguard`/`splitguard`/`hopguard` | bool | 防污染开关 |
...
# 上游探测
./zjdns --probe --pipeline    tcp://8.8.8.8:53       # RFC 7766 管线化
./zjdns --probe --conn-reuse  tls://1.1.1.1:853      # 连接复用
...
| [GUIDELINE.md](docs/rfc/GUIDELINE.md) | RFC 精华指南（关键常量/协议流程/合规状态） |
| [FLOWCHARTS.md](docs/FLOWCHARTS.md) | 架构流程图 |
