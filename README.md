...
支持 JSON 内联规则和 CSV 文件批量导入，O(1) 精确域名匹配 + wildcard `*.domain.com`。

**CSV 文件格式**（`domain, type, content, ttl, rcode`）：

```csv
domain,type,content,ttl,rcode
ads.doubleclick.net.,,,,,                   → 继承 parent 的 response_code
tracker.io.,,,,NXDOMAIN,                    → 自定义 rcode
cdn.cn.,A,10.0.0.1,300,                     → hosts 模式
*.evil.com.,,,,NXDOMAIN                     → wildcard 匹配所有子域名
```
...
```json
"rewrite": [
  { "file": "blocklist.csv", "response_code": 3 },
  { "file": "hosts.csv" },
  { "name": "special.example.com", "records": [{"type": "A", "content": "127.0.0.1"}] }
]
```
...
-- 某域名的请求历史（非命中请求：miss/stale/rewrite/error/blocked/badcookie）
