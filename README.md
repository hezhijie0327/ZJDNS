...
zjdns/
  ...
  │   └── utils.go              # 缓存工具函数
  ├── config/                   # 配置管理模块
  │   ├── config_manager.go     # 配置文件解析和管理
  │   ├── ddr.go                # DDR配置相关
  │   ├── types.go              # 配置类型定义
  │   └── utils.go              # 配置工具函数
  ├── dns/                      # DNS核心功能模块
  │   ├── client.go             # DNS客户端实现
  │   ├── constants.go          # DNS常量定义
  │   ├── hijack_prevention.go  # DNS劫持检测与防护
  │   ├── process.go            # DNS请求处理主流程
  │   ├── query.go              # DNS查询处理
  │   ├── recursive.go          # 递归解析器实现
  │   ├── response.go           # DNS响应处理
  │   ├── rewriter.go           # DNS重写规则处理
  │   ├── server.go             # DNS服务器主逻辑
  │   ├── types.go              # DNS类型定义
  ...
  │   ├── ip_filter.go          # IP过滤器
  │   ├── public_ip_detector.go # 公网IP检测器
  │   └── types.go              # 网络类型定义
  ├── security/                 # 安全相关模块
  │   ├── client.go             # 安全DNS客户端实现
  │   ├── constants.go          # 安全模块常量定义
  │   ├── server.go             # 安全DNS管理器（DoT/DoQ/DoH）
  │   ├── type.go               # 安全模块类型定义
  │   └── utils.go              # 安全模块工具函数
  ├── types/                    # 全局类型定义
  │   ├── constants.go          # 全局常量定义
