...
[![Version](https://img.shields.io/badge/Version-3.6.1-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
- **DNS 防污染**：三层防御体系应对 DNS 劫持/投毒/RST 注入：
  - `spoofguard`（UDP 传输层）：动态静默窗口多读，取最后一个 ID 匹配的响应，转发和递归均生效
  - `poisonguard`（递归模式）：内容分析检测越权 A/AAAA 记录，触发 TCP 回退
  - `splitguard`（TCP 传输层）：TCP 随机分段发送抗 DPI，转发和递归均生效

### DNS 防污染

GFW 通过抢先注入假 DNS 响应（UDP）和 RST 重置（TCP）来干扰 DNS 查询。三层防御覆盖不同攻击面，**推荐三者全开**：

| 机制 | 攻击类型 | 转发⁴ | 递归⁴ | 策略 | 可靠性 |
|------|---------|:------:|:------:|------|:------:|
| `spoofguard` | UDP 欺骗 | ✅ | ✅ | 动态静默窗口多读，尾优先 + richness 回退 | 较高¹ ² ⁵ |
| `poisonguard` | 越权注入 | — | ✅ | 内容验证根/TLD 越权 → TCP 回退 | 高³ |
| `splitguard` | TCP RST | ✅ | ✅ | TCP 帧随机分段发送（大小和延迟均随机） | 高 |

¹ GFW 假包成批先到后静默，真包后到后静默——尾优先，但时序不可靠。

² GFW 假包通常只含 1 个 A 记录，真包通常多个（anycast / CDN）——尾过于贫瘠时回退到 richer 响应。

³ `poisonguard` 作为安全网弥补 `spoofguard` 误判场景，检测越权 → TCP 回退。

⁴ 仅对 Plain DNS 生效。加密传输本身即可防止注入和域名匹配，无需额外配置。
