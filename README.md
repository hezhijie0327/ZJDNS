...
[![Version](https://img.shields.io/badge/Version-3.6.0-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
| `query_stats` | 每日聚合统计（stat_day + result + protocol + rcode + dnssec + poisoned + fallback，滑动窗口 ~500 行） |
...
- `type: "ip"` — CIDR 匹配，启动时加载到二进制 radix trie（`ipTrie`），O(128) 固定时间匹配，与规则数量无关
...
- **DNS 防污染**：三层防御体系应对 DNS 劫持/投毒/RST 注入：
  - `poisonguard`（递归模式）：内容分析检测越权 A/AAAA 记录，触发 TCP 回退
  - `spoofguard`（UDP 传输层）：单 socket 多读 + tail selection，前向和递归 UDP 查询均生效
  - `splitguard`（TCP 传输层）：CTCP 分段发送，前向和递归 TCP 查询均生效

### DNS 防污染

GFW 通过抢先注入假 DNS 响应（UDP）和 RST 重置（TCP）来干扰 DNS 查询。三层防御覆盖不同攻击面：

| 机制 | 攻击类型 | 上游 UDP | 上游 TCP | 递归 |
|------|---------|:--------:|:--------:|:----:|
| `poisonguard` | 越权注入 | — | — | ✅ |
| `spoofguard` | UDP 欺骗 | ✅ | — | ✅¹ |
| `splitguard` | TCP RST | — | ✅ | ✅¹ |

¹ spoofguard 和 splitguard 在递归模式中直接作用于每跳 DNS 查询——spoofguard 在递归 walk 的每个 UDP 查询做 multi-read + tail selection，splitguard 在 TCP 回退时做分段发送。无需等 poisonguard 触发。

加密协议（DoT/DoH/DoQ/TLCP）不受影响——GFW 无法在加密流中注入或匹配域名，无需额外配置。

```json
// 防御机制在 upstream 条目中按需配置：
//   poisonguard — 递归链路内容检测（builtin_recursive）
//   spoofguard  — UDP multi-read 尾部选择（plain UDP）
//   splitguard  — TCP 分段抗 DPI（plain TCP）
{ "address": "builtin_recursive", "poisonguard": true, "splitguard": true },
{ "address": "8.8.8.8:53", "protocol": "udp", "spoofguard": true },
{ "address": "1.2.4.8:53", "protocol": "tcp", "splitguard": true }
```
...
```json
{
  ...
  "upstream": [
    { "address": "builtin_recursive", "poisonguard": true },
...
FROM query_log WHERE poisoned = 1 ORDER BY timestamp DESC;
...
-- 某域名的请求历史（审计日志）
SELECT timestamp, protocol, result, rcode, response_ms, server, poisoned
