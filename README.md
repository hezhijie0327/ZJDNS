...
[![Version](https://img.shields.io/badge/Version-3.5.0-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
- **DNS 防污染**：三层防御体系应对 DNS 劫持/投毒/RST 注入：
  ...
  - `spoofguard`（上游 UDP）：单 socket 多读，取最后到达的响应（GFW 假包先到，真包后到）
  - `splitguard`（plain TCP）：CTCP 分段发送，拆散 DNS 帧使 DPI 无法匹配域名
...
¹ 递归模式中，`poisonguard` 检测到劫持后触发 TCP 回退，回退连接同样受 `splitguard` 保护。

加密协议（DoT/DoH/DoQ/TLCP）不受影响——GFW 无法在加密流中注入或匹配域名，无需额外配置。

```json
"defense": {
  "poisonguard": true,       // 内容检测（递归）
  "spoofguard": true,            // 尾部选择（上游 UDP）
  "splitguard": true     // TCP 分段（plain TCP）
}
```
