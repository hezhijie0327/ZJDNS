...
[![Version](https://img.shields.io/badge/Version-3.3.6-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
高性能递归 DNS 服务器，内置 DNS 劫持/投毒防护、SQLite 缓存、DNSSEC、全协议加密传输 (DoT/DoQ/DoH/DoH3/DoD)、TLCP/DTLCP 国密传输、DNSCrypt 后量子密码学 (X-Wing KEM) 及 KTLS 内核卸载。
...
- **驱逐策略**：TTL 惰性过期 + 条数上限最旧淘汰，`ON DELETE CASCADE` 清理 `ptr_map` + `entry_hit_counters`；无 FK 表（`ip_latency`、`request_log`）通过 30 天时间窗口清理
...
| `protocol` | string | | 传输协议：`udp`(默认)/`tcp`/`tls`(DoT)/`quic`(DoQ)/`https`(DoH)/`http3`(DoH3)/`dod`(DTLS)/`dnscrypt`(DNSCrypt UDP)/`dnscrypt-tcp`(DNSCrypt TCP)/`tlcp`(国密DoT)/`doh-tlcp`(国密DoH)/`dtlcp`(国密DTLS)。使用 `sdns://` 时可省略，自动从戳记推导 |
...
- **安全传输**：DNSCrypt v2（XChacha20-Poly1305 AEAD + 后量子 X-Wing KEM，UDP/TCP 双协议，UDP→TCP 自动回退、PQ 票据续期减少重复 KEM 开销）、DoT (RFC 7858/8310)、DoQ (RFC 9250)、DoH (RFC 8484)、DoH3 (RFC 9114)、DTLS (RFC 8094)，TLS 1.3 + KTLS 可选卸载

### KTLS 内核卸载

将 TLS 加解密从 CPU 卸载至 Linux 内核（`af_alg` + `setsockopt(TCP_ULP)`），降低 CPU 占用。仅适用 DoT/DoH（TCP），QUIC 不适用。非 Linux 静默回退。
...
> `"bad record MAC"` 错误 → 关闭 `kernel_rx`。客户端上游 DoT/DoH 连接自动复用服务端 KTLS 配置。
...
**服务端配置**（复用 SM2 双证书，与 TLCP DoT/DoH 共用）：
...
| [7858](https://www.rfc-editor.org/info/rfc7858) / [8310](https://www.rfc-editor.org/info/rfc8310) | DNS over TLS | DoT TLS 1.3 + Strict/Opportunistic 隐私配置文件 |
...
| [9000](https://www.rfc-editor.org/info/rfc9000) | QUIC | DoQ 传输层 |
| [9018](https://www.rfc-editor.org/info/rfc9018) | DNS Cookies | SipHash-2-4 服务器 Cookie（时间戳 + 版本字段，互操作格式） |
| [9114](https://www.rfc-editor.org/info/rfc9114) | HTTP/3 | DoH3 传输层 |
| [9156](https://www.rfc-editor.org/info/rfc9156) | QNAME Minimisation | 递归查询名最小化 |
| [9250](https://www.rfc-editor.org/info/rfc9250) | DNS over QUIC | DoQ 协议映射 |
...
| [GB/T 38636-2020](https://std.samr.gov.cn/gb/search/gbDetailed?id=A47A713B764314ABE05397BE0A0ABB25) | TLCP (国密 SSL) | SM2/SM3/SM4 密码套件 (ECC/ECDHE_SM4_GCM/CBC_SM3)，DoT/DoH 传输 |
| [GM/T 0128-2023](https://std.samr.gov.cn/hb/search/stdHBDetailed?id=1BF26B7A9FF0FD76E06397BE0A0A81D8) | DTLCP (国密 DTLS) | SM2/SM3/SM4 over UDP，DNS-over-DTLCP 传输 |
