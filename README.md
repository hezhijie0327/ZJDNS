...
[![Version](https://img.shields.io/badge/Version-3.2.12-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
- `type: "domain"` — 域名后缀匹配（`google.com` 匹配自身及所有子域，`*.youtube.com` 只匹配子域），O(1) TLD+1 map 查找
- `type: "ip"` — CIDR 匹配，binary trie O(128)=O(1) 查找
- `file` — 每行一个条目，按 `type` 解析；`#` 注释
- `match` 支持 `!` 取反：`["google", "!block-ip"]` 表示匹配 google 标签但不含被屏蔽 IP
