...
[![Version](https://img.shields.io/badge/Version-3.3.3-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
高性能递归 DNS 服务器，内置 DNS 劫持/投毒防护、SQLite 缓存、DNSSEC、全协议加密传输 (DoT/DoQ/DoH/DoH3/DTLS)、TLCP 国密传输、DNSCrypt 后量子密码学 (X-Wing KEM) 及 KTLS 内核卸载。
...
| `protocol` | string | | 传输协议：`udp`(默认)/`tcp`/`tls`(DoT)/`quic`(DoQ)/`https`(DoH)/`http3`(DoH3)/`dtls`(DTLS)/`dnscrypt`(DNSCrypt UDP)/`dnscrypt-tcp`(DNSCrypt TCP)/`tlcp`(国密DoT)/`doh-tlcp`(国密DoH)。使用 `sdns://` 时可省略，自动从戳记推导 |
...
| [GB/T 38636-2020](https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=778097598DA2761E94A5FF3F77BD66DA) | TLCP (国密 SSL) | SM2/SM3/SM4 密码套件 (ECC/ECDHE_SM4_GCM/CBC_SM3)，DoT/DoH 传输 |
