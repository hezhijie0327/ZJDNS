...
[![Version](https://img.shields.io/badge/Version-3.2.14-informational)](https://github.com/hezhijie0327/ZJDNS/releases)
...
| `nsec_chain` | NSEC/NSEC3 否定缓存索引（wire-format BLOB，FK→entries ON DELETE CASCADE） |
| `infra_cache` | 权威服务器状态（RTT、EDNS 能力、超时退避） |
| `ruleset_entries` | 规则集条目（tag + type + value，启动时加载至内存 trie/map） |
...
- `type: "domain"` — 域名后缀匹配（`google.com` 匹配自身及所有子域，`*.youtube.com` 只匹配子域），O(1) TLD+1 map 查找，规则存储在 SQLite `ruleset_entries` 表中
- `type: "ip"` — CIDR 匹配，binary trie O(128)=O(1) 查找
...
- **Aggressive NSEC 负缓存**（RFC 8198）：已验证响应的 NSEC/NSEC3 记录索引至 SQLite，后续查询直接合成 NXDOMAIN/NODATA，省去上游查询
...
- **0x20 查询名大小写随机化**：随机化对外查询 qname 的大小写，响应必须保持相同模式——盲伪造者无法预测随机大小写，CAPSFAIL 自动回退无大小写重试
...
| [2308](https://www.rfc-editor.org/info/rfc2308) | Negative Caching | SOA 负面 TTL 缓存 + NSEC/NSEC3 TTL 封顶 (10800s) |
...
| [8198](https://www.rfc-editor.org/info/rfc8198) | Aggressive NSEC Caching | NSEC/NSEC3 负缓存索引 + NXDOMAIN/NODATA 本地合成 |
