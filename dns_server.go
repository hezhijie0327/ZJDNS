...
func NewDNSServer(config *ServerConfig) (*RecursiveDNSServer, error) {
	...
	speedTester := NewSpeedTester(*config) // 使用配置创建SpeedTester
...
func (r *RecursiveDNSServer) shutdownServer() {
	...
	if r.speedTester != nil {
		// 忽略关闭错误
		_ = r.speedTester.Close()
		writeLog(LogInfo, "📍 速度测试器已关闭")
	}
...
func (r *RecursiveDNSServer) displayInfo() {
	...
	// 添加路由检测功能状态的显示
	if len(r.config.Speedtest) > 0 { // 修改这一行
...
func (r *RecursiveDNSServer) processQuerySuccess(req *dns.Msg, question dns.Question, ecsOpt *ECSOption,
	...
	if len(r.config.Speedtest) > 0 { // 修改这一行
		...
		// 检查是否需要执行速度测试（防抖机制）
		shouldCheckRoute := r.shouldCheckRoute(question.Name)
		if shouldCheckRoute {
			...
			r.taskManager.ExecuteAsync(fmt.Sprintf("speed-test-%s", question.Name), func(ctx context.Context) error {
				...
				// 执行速度测试和排序
				r.speedTester.CheckAndSortRoutes(msgCopy)
...
// shouldCheckRoute 检查是否应该对域名进行速度测试（防抖机制）
func (r *RecursiveDNSServer) shouldCheckRoute(domain string) bool {
